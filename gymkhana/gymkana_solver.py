...
class GymkhanaSolver:
    ...
    def solve_gymkhana(self):
        ...
        try:
            self.gui.update_progressbar_message("Solving step 0...", )
            code_step1 = self.step0.run()

            self.gui.update_progressbar_message("Solving step 1...")
            code_step2 = self.step1.run(code_step1)

            self.gui.update_progressbar_message("Solving step 2...")
            code_step3 = self.step2.run(code_step2)

            self.gui.update_progressbar_message("Solving step 3...")
            code_step4 = self.step3.run(code_step3)

            self.gui.update_progressbar_message("Solving step 4...")
            code_step5 = self.step4.run(code_step4)

            self.gui.update_progressbar_message("Solving step 5...")
            self.step5.run(code_step5)

            self.gui.update_progressbar_message("Gymkhana completed!")
